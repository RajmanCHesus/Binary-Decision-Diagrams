...
void init_unique_table(int size) {
    ...
    if (!unique_table.buckets) {
        fprintf(stderr, "Memory allocation failed for unique table\n");
        exit(1);
    }
}
...
int hash_node(int var, Node *low, Node *high) {
    unsigned long hash = (unsigned long) var * 101 + (unsigned long) low * 1009 + (unsigned long) high * 10007;
...
Node *create_terminal(int value) {
    if (value == 0) {
        if (zero_terminal == NULL) {
            ...
            if (!zero_terminal) {
                fprintf(stderr, "Memory allocation failed for terminal node\n");
                exit(1);
            }
            zero_terminal->var = -1;
...
Node *find_or_add_node(int var, Node *low, Node *high) {
    ...
    // Terminal case optimization: if both children are the same, return the child directly
    if (low == high) {
        return low;
    }
...
int count_variables(const char *bfunkcia) {
    ...
    for (int i = 0; bfunkcia[i] != '\0'; i++) {
        if ((bfunkcia[i] >= 'A' && bfunkcia[i] <= 'Z') ||
            ...
            if (var_idx > max_var) max_var = var_idx;
        }
    }
...
Node *create_minterm_bdd(int input_combination, int num_vars, const char *var_order) {
...
Node *apply_or(Node *f, Node *g) {
    ...
    if (f->var == -1 && f->value == 1) return f;
    if (g->var == -1 && g->value == 1) return g;
    if (f->var == -1 && f->value == 0) return g;
    if (g->var == -1 && g->value == 0) return f;
...
int count_nodes(Node *root, int *visited, int next_id) {
    if (root == NULL) return next_id;
    if (root->var == -1) return next_id; // Don't count terminal nodes
...
void free_unique_table() {
    // Safety check
    if (unique_table.buckets == NULL) return;
...
BDD *BDD_clone(BDD *source) {
    ...
    if (!new_bdd->var_order) {
        fprintf(stderr, "Memory allocation failed for variable ordering in clone\n");
        free(new_bdd);
        return NULL;
    }
...
void test_bdd() {
    ...
    const int max_vars = 6; // Reduced for testing, increase up to 13 for final version
    ...
    for (int num_vars = 3; num_vars <= max_vars; num_vars++) {
        ...
        int num_tests = 10; // Reduced for testing, increase to 100 for final version

        printf("Testing with %d variables, %d random functions...\n", num_vars, num_tests);

        for (int i = 0; i < num_tests; i++) {
            // Generate a random Boolean function
            char *function = generate_random_boolean_function(num_vars, rand() % 3 + 1);
            ...
            printf("Function %d: %s, Direct: %d nodes, Best: %d nodes\n",
                   i + 1, function, direct_size, bdd_best->size);
